...
namespace ConsoleApp1
{
    ...
    public class cpuschedule //https://www.geeksforgeeks.org/gate-notes-operating-system-process-scheduling/
    {
        ...
        public void Fcfs()
        {
            ...
            float averageTurnAroundTime = tat.Sum() / n;
        ...
        public void Priority()
        {
            ...
            for (int i = 0; i < n; i++)
            {
                //int index = p.ToList().IndexOf(pri[i]);
                //if (i > 0)
                //{
                //    if ((pri[i] == pri[i - 1]) )
                //    {
                ...
                //}
                //Twt = Twt + Wt[i];
                //tat[index] = B[index] + Wt[i] - A[index];


                awt = awt + "," + wtt[i].ToString();
                att = att + "," + tt[i].ToString();
            }
            float averageTurnAroundTime =(float) tt.Sum() / n;
            Awt = (float)wtt.Sum() / n;
            Console.WriteLine("waiting time are " + awt + "\n Turn around time are " + att);
            Console.WriteLine("Total Weighting Time={0}\nAverage Weighting Time={1}", averageTurnAroundTime, Awt);
            Exit();
        }
